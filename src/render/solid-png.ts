...
export const solidPngDims = (w: number, h: number): { w: number; h: number } => {
  const scale = MAX_SIDE / Math.max(w, h, 1);
  return { w: Math.max(1, Math.round(w * scale)), h: Math.max(1, Math.round(h * scale)) };
};

export const solidPngBytes = (hexColor: string, dims: { w: number; h: number }): Buffer => {
  const rgb = /^#?([0-9a-fA-F]{6})$/.exec(hexColor)?.[1];
  if (rgb === undefined) throw new Error(`solid PNG needs #rrggbb, got "${hexColor}"`);
...
export const solidPngFile = (hexColor: string, w: number, h: number): string => {
  ...
  if (!existsSync(file)) writeFileSync(file, solidPngBytes(hexColor, dims));
  return file;
};
