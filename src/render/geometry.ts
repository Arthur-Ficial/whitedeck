...
const area = (r: Rect): number => Math.max(r.w, 0) * Math.max(r.h, 0);

const clamp = (r: Rect, bounds: Rect): Rect => {
 ...
 */
export const carveExported = (frame: Rect, obstacle: Rect, gap: number): Rect => carve(frame, obstacle, gap);

const carve = (frame: Rect, obstacle: Rect, gap: number): Rect => {
  if (!intersects(frame, obstacle)) return frame;
 ...
 */
export const picFrame = (ph: ThemePlaceholder, layout: ThemeLayout): Rect => {
  const bounds = canvas();
  ...
  if (!beside) {
    const column = texts.reduce<Rect | undefined>(
      (widest, t) => (widest === undefined || t.w > widest.w ? t : widest),
      undefined,
    );
  ...
  for (const t of texts) frame = carve(frame, t, TEXT_GAP_EMU);
...
export const pngSize = (path: string): ImageSize | undefined => {
  let head: Buffer;
  try {
...
export const jpegSize = (path: string): ImageSize | undefined => {
  let data: Buffer;
  try {
  ...
  while (offset + 9 < data.length) {
    ...
    if (isSof(marker)) return { h: data.readUInt16BE(offset + 5), w: data.readUInt16BE(offset + 7) };
...
export const imageSize = (path: string): ImageSize | undefined => pngSize(path) ?? jpegSize(path);

/** Scale the image down into the frame, preserving aspect ratio, centred. */
export const fitted = (path: string, frame: Rect): Rect => {
 ...
 */
export const sourceFrame = (layout: ThemeLayout, bodyUsed: boolean): Rect => {
  ...
  const after =
    bodyUsed && body !== undefined ? body.yEmu + body.hEmu + TEXT_GAP_EMU : Number.POSITIVE_INFINITY;
  return {
...
export const insideCanvas = (r: Rect): boolean =>
  r.x >= 0 && r.y >= 0 && r.x + r.w <= WHITE.canvas.widthEmu && r.y + r.h <= WHITE.canvas.heightEmu;

export const intersects = (a: Rect, b: Rect): boolean =>
  a.x < b.x + b.w && b.x < a.x + a.w && a.y < b.y + b.h && b.y < a.y + a.h;

export const rectOf = (p: ThemePlaceholder): Rect => ({
  x: p.xEmu,
  y: p.yEmu,
  w: p.wEmu,
  h: p.hEmu,
 ...
 */
export const imageBandFrame = (layout: ThemeLayout, hasSource: boolean): Rect => {
  const bounds = canvas();
  const body = placeholdersByRole(layout, 'body')[0];
  const title = placeholdersByRole(layout, 'title')[0];
  const base: Rect =
  ...
  const frame: Rect = { x: base.x, y: base.y, w: base.w, h: Math.max(bottom - base.y, 1) };
...
export const bodyFrame = (layout: ThemeLayout, hasSource: boolean): Rect => {
  const body = placeholdersByRole(layout, 'body')[0];
  if (body === undefined) return { x: 0, y: 0, w: 0, h: 0 };
