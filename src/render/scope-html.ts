...
import { BORDER_PT, ptToPx, type PtRect, type PtText } from '../theme/scope.js';
...
const boxStyle = (r: PtRect): string =>
  `position:absolute;left:${ptToPx(r.x)}px;top:${ptToPx(r.y)}px;width:${ptToPx(r.w)}px;height:${ptToPx(r.h)}px;`;

const textStyle = (t: PtText): string =>
  `${boxStyle(t)}font-size:${t.sizePt}pt;font-weight:${FONT_WEIGHT[t.font]};color:${t.color};text-align:${t.align};` +
...
const rectHtml = (rect: PtRect, fill: string): string =>
  `<div class="wd-rect" style="${boxStyle(rect)}background:${fill};"></div>`;

interface ListStyle {
  readonly indentPx: number;
  readonly gapPt: number;
  readonly pct: number;
  /** First item flush to the top (a body list) or spaced like the rest (a notes group). */
  readonly flush: boolean;
}

const listHtml = (items: readonly { text: string; level: number }[], { indentPx, gapPt, pct, flush }: ListStyle): string => {
  const li = items
    .map(
      (b) =>
        `<li style="padding-left:${indentPx * (b.level + 1)}px;text-indent:-${indentPx}px;margin-top:${gapPt}pt;">` +
...
const bulletsHtml = (p: PlacedBullets): string =>
  `<div class="wd-text" style="${textStyle(p.box)}">${listHtml(p.bullets, { indentPx: ptToPx(p.indentPt), gapPt: p.spaceBeforePt, pct: p.bulletSizePct, flush: true })}</div>`;

const notesHtml = (p: PlacedNotes): string => {
...
const placedHtml = (p: Placed, marpSrc: (path: string) => string): string => {
  if (p.kind === 'text') return textHtml(p);
  if (p.kind === 'image') return imageHtml(p, marpSrc);
  if (p.kind === 'rect') return rectHtml(p.rect, p.fill);
  if (p.kind === 'bullets') return bulletsHtml(p);
  return notesHtml(p);
};

/** One raw-HTML block for the whole paint list; `marpSrc` resolves image paths for Marp. */
export const placedToHtml = (placed: readonly Placed[], marpSrc: (path: string) => string): string =>
  `<div class="wd-custom">${placed.map((p) => placedHtml(p, marpSrc)).join('')}</div>`;

/** Theme CSS the custom layouts rely on (fonts, list reset, bullet glyph). */
export const customLayoutCss = (): string =>
  [
