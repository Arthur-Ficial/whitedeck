...
import { BORDER_PT } from '../theme/scope.js';
...
const rgb = (hexColor) => {
    ...
    if (m === null)
        throw new Error(`colour must be #rrggbb, got "${hexColor}"`);
...
let counter = 0;
const fresh = (prefix) => `${prefix}${(counter += 1)}`;
const imageAt = (path, r) => {
    const v = fresh('im');
    return [
...
const rectAt = (r, fill) => imageAt(solidPngFile(fill, r.w, r.h), r);
/** Styled runs of one paragraph, offset by `base` characters (bold font, colour). */
export const runStatements = (v, text, base) => styledRuns(text).flatMap((run) => {
...
const paintText = (p) => textItem(p.box, p.text);
const paintImage = (p) => {
    const half = BORDER_PT / 2;
    const border = p.border === undefined
        ? []
...
const notesLines = (columns) => columns.flatMap((col) => [col.header, ...col.bullets.map((b) => `• ${b.text}`)]);
const paintNotes = (p) => {
    const v = fresh('ti');
    ...
    for (const line of lines) {
        ...
        if (isHeading)
            runs.push(`set font of characters ${base + 1} thru ${base + visible.length} of object text of ${v} to ${str(POSTSCRIPT.bold)}`);
...
const paintBullets = (box, bullets) => {
...
export const placedStatements = (placed) => placed.flatMap((p) => {
    if (p.kind === 'text')
        return paintText(p);
    if (p.kind === 'image')
        return paintImage(p);
    if (p.kind === 'rect')
        return rectAt(p.rect, p.fill);
    if (p.kind === 'notes')
        return paintNotes(p);
