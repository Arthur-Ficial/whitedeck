...
export const slugify = (value) => {
    ...
    for (const word of asciiWords(value)) {
        ...
        if (length + added > MAX_SLUG_LENGTH)
            break;
        kept.push(word);
        length += added;
    }
...
const deckTitle = (deck) => deck.meta.title ?? deck.slides.find((slide) => slide.title !== undefined)?.title;
/**
 ...
 */
export const deckFileBase = (deck, inputName) => {
 ...
 * pointed at.
 */
export const checkedBaseName = (name) => {
    if (/[/\\]/.test(name) || name === '.' || name === '..') {
        throw new Error(`Output name "${name}" is a path - pass a plain file name and use -o for the folder.`);
    }
    if (name.trim() === '')
        throw new Error('Output name is empty - pass a file name or drop --name.');
    return name;
};
